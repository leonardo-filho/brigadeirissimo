export const menuData = {
  tradicionais: {
    title: "Sabores Tradicionais",
    flavors: [
      "Brigadeiro (tradicional)", "Ninho", "Casadinho (brigadeiro & ninho)",
      "Napolitano", "Beijinho", "Nesquik"
    ],
    prices: [
      { item: "Cento (até 4 sabores)", price: "R$ 170,00" },
      { item: "Meio cento (até 2 sabores)", price: "R$ 90,00" },
      { item: "25 unidades", price: "R$ 55,00" },
      { item: "Unidade", price: "R$ 3,00" }
    ]
  },
  especiais: {
    title: "Sabores Especiais",
    flavors: [
      "Ninho com Nutella", "Brigadeiro (granulado Melken)", "Romeu & Julieta",
      "Churros", "Maracujá", "Limão", "Oreo", "Surpresa de Uva",
      "Amendoim", "Ferrero Rocher", "Caramelo", "Prestígio",
      "Ovomaltine", "Café", "Paçoca"
    ],
    prices: [
      { item: "Cento (até 4 sabores)", price: "R$ 200,00" },
      { item: "Meio cento (até 2 sabores)", price: "R$ 110,00" },
      { item: "25 unidades", price: "R$ 70,00" },
      { item: "Unidade", price: "R$ 3,50" }
    ]
  },
  info: [
    "Docinhos de 15g (cento/meio) ou 17g (25 unid).",
    "Encomendas com no mínimo 48h de antecedência.",
    "Confirmação mediante 50% do valor.",
    "Caixas de cento acompanham pétalas."
  ]
};